...
/* ===== Recaptcha ↔ Modal interaction handler =====
   - Close modal when user clicks the recaptcha widget
   - Re-open modal when captcha succeeds or popup removed
   - Works with DaisyUI <dialog> modals (showModal/close)
*/

(function(){
  const modal = document.getElementById('orderModal'); // তোমার modal ID
  const form = document.getElementById('orderForm');
  const captcha = document.querySelector('.g-recaptcha');

  if(!modal || !captcha) return; // safety

  let recaptchaClicked = false;
  let observer = null;

  // 1) Detect initial click on the visible recaptcha widget (user intent to solve)
  captcha.addEventListener('pointerdown', () => {
    recaptchaClicked = true;
    // close modal so the recaptcha popup is not overlapped
    try { if (typeof modal.close === 'function') modal.close(); } catch(e) {}
    // small delay to allow popup to render (optional)
    setTimeout(()=>{/* noop */}, 120);
  });

  // Also listen for keyboard activation (Enter/Space)
  ...
  // 2) MutationObserver: watch for recaptcha popup (challenge) added/removed
  //    Popup uses elements like .rc-imageselect, .rc-dialog, iframe[src*=recaptcha]
  const popupSelector = '.rc-imageselect, .rc-dialog, .rc-popup, iframe[src*="recaptcha"]';

  observer = new MutationObserver((mutationsList) => {
    // if any popup-like element present -> ensure modal closed
    const popupPresent = !!document.querySelector(popupSelector);

    if(popupPresent) {
      // if popup exists, ensure modal closed
      try { if (typeof modal.close === 'function') modal.close(); } catch(e) {}
    } else {
      // popup removed; re-open modal if we had closed it due to recaptcha click
      if(recaptchaClicked) {
        ...
        setTimeout(() => {
          try {
            if (typeof modal.showModal === 'function') {
              ...
              // fallback for non-dialog modals: remove hidden class etc.
  ...
  // start observing body for additions/removals deep in DOM
  observer.observe(document.body, { childList: true, subtree: true });

  // 3) grecaptcha callback (on success) -> reopen modal and attach token to form
  window.onRecaptchaSuccess = window.onRecaptchaSuccess || function(token){
    // Add/Set hidden input with token (Formspree expects g-recaptcha-response)
    if(form){
    ...
    // reopen modal after small delay
    setTimeout(() => {
      try {
  ...
  // 4) grecaptcha expired callback -> reopen if needed
  window.onRecaptchaExpired = window.onRecaptchaExpired || function(){
    // token expired; clear hidden input
    if(form){
    ...
    setTimeout(() => {
      try { if(typeof modal.showModal === 'function') modal.showModal(); } catch(e){ }
      recaptchaClicked = false;
    }, 150);
  };

  // 5) Cleanup on modal close by user (reset recaptcha if they close manually)
